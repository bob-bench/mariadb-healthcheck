...
func checkMysql(user string, password string, host string, port int) (int, string) {
	...
	rows, err := db.Query("SHOW GLOBAL STATUS LIKE 'wsrep_ready'")
	if err != nil {
		db.Close()
		fmt.Print(err, "\n")
...
func main() {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, req *http.Request) {
		code, reason := checkMysql("user", "pass", "localhost", 3306)
		w.WriteHeader(code)
		w.Write([]byte(reason))
	})
